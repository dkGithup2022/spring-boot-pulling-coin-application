...
import com.example.pullingcoinapplication.repository.upbitCandleRepository.*;
...
public class UpbitCandleService {
    ...
    public UpbitCandle safeSave(UpbitCandle candle){
        if(findOneByMarketAndTimestamp(UpbitCodeUtil.getCoinCodeFromString(candle.getMarket()), candle.getTimestamp()) ==null ){
            return save(candle);
        }else{
           return  null;
        }
...
class CandleRepositoryPicker {
    ...
    public UpbitCandleCommonJpaInterface pickRepository(UpbitCandle candle) {
        return getRepositoryFromCode(UpbitCodeUtil.getCoinCodeFromString(candle.getMarket()));
    }
